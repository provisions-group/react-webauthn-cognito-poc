...
// functions/define-auth-challenge.js
var _ = require_lodash();
var { MAX_ATTEMPTS } = require_constants();
module.exports.handler = async (event) => {
  ...
  if (event.request.session && event.request.session.find((attempt) => attempt.challengeName !== "CUSTOM_CHALLENGE")) {
    ...
    event.response.issueTokens = false;
    event.response.failAuthentication = true;
  } else if (attempts >= 1 && lastAttempt.challengeName === "CUSTOM_CHALLENGE" && lastAttempt.challengeResult === true) {
