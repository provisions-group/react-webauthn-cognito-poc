...
Amplify.configure({
  Auth: {
    region: "us-east-2",
    userPoolId: "us-east-2_56O5hbcfY",
    userPoolWebClientId: "4lef5ufjg433ngi9nkvdadkf6i",
...
async function answerCustomChallenge() {
  ...
  try {
    ...
    if (challengeResult.challengeName) {
      secretCode.value = "";
      attemptsLeft.value = parseInt(
        challengeResult.challengeParam.attemptsLeft
      );

      alert(
        `The code you entered is incorrect. ${attemptsLeft.value} attempts left.`
      );
    } else {
...
<template>
  ...
  <div>
    <h1 class="font-bold text-4xl mb-12">
      Passwordless authentication with OTP with Cognito
    </h1>
    ...
    <div v-if="isSignedIn" class="w-1/2 flex-row mb-10">
      <h2 class="font-semibold text-xl">
        Step 3. you're signed in! Sign out and try again.
      </h2>
